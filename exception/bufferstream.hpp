...
#ifndef _part_BOOST_INTERPROCESS_BUFFERSTREAM_HPP
#define _part_BOOST_INTERPROCESS_BUFFERSTREAM_HPP
...
public:
    ...
    //!Does not throw.
    std::pair<CharT *,std::size_t> buffer() const { return std::pair<CharT *,std::size_t>(m_buffer,m_length); }

    //!Sets the underlying buffer to a new value
    //!Does not throw.
    void buffer(CharT *buf,std::size_t length) { m_buffer=buf;   m_length=length;   this->set_pointers(); }
...
public:
    ...
    //!stream buffer.
    basic_bufferbuf<CharT,CharTraits>* rdbuf() const { return const_cast<basic_bufferbuf<CharT,CharTraits>*>(&get_buf()); }

    //!Returns the pointer and size of the internal buffer.
    //!Does not throw.
    std::pair<const CharT *,std::size_t> buffer() const { return get_buf().buffer(); }

    //!Sets the underlying buffer to a new value. Resets
    //!stream position. Does not throw.
    void buffer(const CharT *buf,std::size_t length) { get_buf().buffer(const_cast<CharT*>(buf),length); }
};
...
#if !defined(BOOST_INTERPROCESS_DOXYGEN_INVOKED)
    public basic_bufferbuf<CharT,CharTraits>,
#endif   //#ifndef BOOST_INTERPROCESS_DOXYGEN_INVOKED
...
public:
    ...
    //!Returns the pointer and size of the internal buffer.
    //!Does not throw.
    std::pair<CharT *,std::size_t> buffer() const { return get_buf().buffer(); }

    //!Sets the underlying buffer to a new value. Resets
    //!stream position. Does not throw.
    void buffer(CharT *buf,std::size_t length) { get_buf().buffer(buf,length); }
};
